...
var validateConstant = {
	"notEmpty" : /^.+$/,// legal characters
	"password" : /^[0-9A-Za-z]{1,18}$/,// password
	"rightfulString" : /^[A-Za-z0-9_-]+$/,// legal characters
	"number" : /^\d+$/,// number
	"endlish" : /^[A-Za-z]+$/,// English alphabet 
	"numberEnglish" : /^[A-Za-z0-9]+$/,// English alphabet and number
	"float" : /^[+]?\d+(\.\d+)?$/,// floating type
	"money" : /(^[1-9]\d{0,9}(\.\d{1,2})?$)/,
	// "chinese" : "/^[\u4e00-\u9fa5]+$/",// 纯中文
	"mobile" : /^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1})|(17[0-9]{1})|(14[0-9]{1}))+\d{8})$/,// phone number
	"tel" : /^(\d{3,4}-?)?\d{7,9}$/g,// telephone number
	// "qq" : /^[1-9]\d{4,12}$/,// qq
	"zipCode" : /^[0-9]{6}$/,// Post code
	"email" : /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/,// Email
	"positive":/^[1-9][0-9]+$/,// Number > 0
...
var errorMsg = {
	...
	"password" : "Please use the correct pin",
	"rightfulString" : "Please input the right characters",// 合法字符
	"number" : "Only Number",// 数字
	"endlish" : "Only English",// 纯英文
	"numberEnglish" : "Only Alphabet and Number",// 英文和数字
	"float" : "Only Decimal",// 浮点型
	"money" : "Please write the right price",
	//"chinese" : "只能输入汉字",// 纯中文
	"mobile" : "Please write the right Cell-Phone number",// 手机号
	"tel" : "Please write a right Telephone number",// 电话
	//"qq" : "请输入正确的QQ号",// qq
	"zipCode" : "Please write the right post code",
	"email" : "Please write the right email address",// 邮箱
	"positive":"Number must bigger than 0",//大于0的数字
